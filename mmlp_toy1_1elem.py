...
Toy 1 : Apples and Oranges 
      
    Adapted from Chapter 3 of "Neural Network Design" by Hagan, Demuth, Beale 
    where orange is =[1] and apple is = [0] - absolute simplest case
...
for l in range(1,2): 
    ...
    netLyr[l].mn.append(multiplet_neuron(netLyr[l].wtgrp[0],-3,1,np.random.laplace(0.5,0.3),0.0)) # "minimum-ish" ( error results with -4 or -5 - don't use)
    netLyr[l].mn.append(multiplet_neuron(netLyr[l].wtgrp[0],1,1,np.random.laplace(0.5,0.3),0.0))  # "dot product"
    netLyr[l].mn.append(multiplet_neuron(netLyr[l].wtgrp[0],5,1,np.random.laplace(0.5,0.3),0.0)) # "max-ish"
...
[1. 0.]
------- layer ------  1  --------
neuron 0 aout=-0.06 on p=-3 b=-0.15 m=0.94
      w= [1.]
neuron 1 aout=-0.08 on p=1 b=-0.15 m=0.70
      w= [1.]
neuron 2 aout=-0.04 on p=5 b=-0.15 m=1.10
      w= [1.]
------- layer ------  2  --------
neuron 0 aout=-0.00 on p=1 b=0.07 m=1.22
      w= [1.    1.003 0.997]
neuron 1 aout=1.00 on p=1 b=0.93 m=-1.22
      w= [1.    1.003 0.997]
output from input vector  [0.1]  is
[-0.  1.]  
...
output from input vector  [1.0]  is
[1.022 0.074]
------- layer ------  1  --------
neuron 0 aout=0.24 on p=-3 b=0.13 m=1.11
      w= [1.]
neuron 1 aout=-0.07 on p=1 b=-0.16 m=0.92
      w= [1.]
neuron 2 aout=-0.01 on p=5 b=-0.10 m=0.96
      w= [1.]
------- layer ------  2  --------
neuron 0 aout=-0.03 on p=1 b=0.02 m=1.25
      w= [0.    3.283 2.626]
neuron 1 aout=0.90 on p=1 b=0.86 m=-0.98
      w= [0.    3.283 2.626]
output from input vector  [0.1]  is
[-0.029  0.898]
