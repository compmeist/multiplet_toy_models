...
Toy 1: Identiy function, simple case with two input element vector
...
class multiplet_neuron():
    ...
    def calcOut(self,xv,L):    
# calc the numerator and denominator sigmas 
#        print(xv.x[self.p+5])
#        print(xv.x[self.p_q+5])

        self.numer = np.vdot(np.power(self.wPtr.w,L),xv.x[self.p+5])
        self.denom = np.vdot(np.power(self.wPtr.w,L),xv.x[self.p_q+5])
#        print(self.numer)
#        print(self.denom)
        if (self.denom != 0.0):
    ...
    def calcPartialDers(self,xv,L):
        ...
        if (self.denom != 0.0):
            ...
            newW1 = np.power(self.wPtr.w,(L-1))
            print(newW1)
            newW = np.power(self.wPtr.w,L)
            self.ddw = (m_denom2 * L) * (newW1 * (self.denom * xv.x[self.p+5] - self.numer * xv.x[self.p_q+5]))
            ...
            self.ddx =  m_denom2 * (  (self.denom * self.p) * np.multiply(newW,xv.x[(self.p-1)+5])
                - ((self.numer * self.p_q) * np.multiply(newW,xv.x[(self.p_q-1)+5])))
...
def forwardPassInstance(inputVector,targets,wt_power):
    ...
    for l in range(1,len(netLyr)): # start with layer 1
        ...
        for lk1 in range(len(netLyr[l].mn)):  # traverse through layer neurons
            netLyr[l].mn[lk1].calcOut(xAryj,wt_power)  # forward prop
            # this step is not in normal Multlayer Perceptrons, since it is such a straightforward deriviative
            # ( no activations here)
            netLyr[l].mn[lk1].calcPartialDers(xAryj,wt_power)  
...
for k in range(30):
    ...
    for ii in range(len(xAryIn)):
        ...
        forwardPassInstance(xAryIn[ii],yTargets[ii],4)
