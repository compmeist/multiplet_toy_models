...
# this works with above input vector/tensor
    ...
    def chkWtContraints(self):
        ...
        # if (cwcmaxVal > 100.0):
        #     self.w -= ((cwcminVal + cwcmaxVal) * w_alpha)
        if (cwcminVal < 0.0):
            self.w += np.abs(cwcminVal) 
        if (cwcmaxVal > 0.0):
            self.w /= cwcmaxVal;   # in lieu of L2 or something

    def calcCSlpScore(self,xv):
        numer1 = np.vdot(np.power(self.w,0),xv.x[6+5])
        denom1 = np.vdot(np.power(self.w,0),xv.x[5+5])
        numer2 = np.vdot(np.power(self.w,0),xv.x[-3+5])
        denom2 = np.vdot(np.power(self.w,0),xv.x[-4+5])
        if ((denom1 != 0.0) and (denom2 != 0.0)):
            self.cSlpScore = math.tanh(numer1/denom1 + numer2/denom2)
#       

    def applyWDeltas(self,w_nu):
        ddE = self.errSum
        self.w -= ((w_nu * self.cSlpScore) * ddE) * self.ddw   #  * actTerm
...
#  
        self.numer = np.vdot(np.power(self.wPtr.w,self.p),xv.x[self.p+5])
        self.denom = np.vdot(np.power(self.wPtr.w,self.p_q),xv.x[self.p_q+5])
#        print(self.numer)
#        print(self.denom)
        if (self.denom != 0.0):
    ...
    def calcPartialDers(self,xv):
        ...
        if (self.denom != 0.0):
            ...
            newWpq1 = np.power(self.wPtr.w,(self.p_q-1))
            newWp1 = np.power(self.wPtr.w,(self.p-1))
            # vector ddw = scalar * (vector - vector)
            # with repect to w_k, but for all k - so a vector 
            
            # self.ddw = xv.x[1+5]   # this is just x_i
            ...
            self.ddx =  (self.m * (self.p / self.denom) * np.multiply(newWp1,xv.x[(self.p-1)+5])) - m_denom2 * ((self.numer * self.p_q) * np.multiply(newWpq1,xv.x[(self.p_q-1)+5]))
            self.ddw = self.ddx

            self.ddx *= xv.x[1+5]
            self.ddw *= self.wPtr.w 
            # self.ddx /= self.normW


    def applyPtLearnNoActvtn(self,b_nu,m_nu,w_nu):
        #ddE = self.deltaE
        ddE = self.ss
        ...
        self.wPtr.w -= ((w_nu * self.wPtr.cSlpScore) * ddE) * self.ddw   #  * actTerm  
...
def forwardPassInstance(inputVector,targets):
    ...
    for l in range(1,len(netLyr)): # start with layer 1
        ...
        for wk1 in range(len(netLyr[l].wtgrp)):
            netLyr[l].wtgrp[wk1].calcCSlpScore(xAryj)
...
# the w_i total error is accumulated within the weight_vec class, but idk if it really matters, since it is an error sum anyway
    ...
    #     the weights w have a separate variable, since shared
    # for wk1 in range(len(netLyr[l].wtgrp)):    
    #     netLyr[l].wtgrp[wk1].clearErrSum()   

    for lk1 in range(len(netLyr[l].mn)):  # output layer neurons
        ddxout = (netLyr[l].mn[lk1].aout - targets[lk1]) #derviative of MSE
        # print('ddxout is ',ddxout)
        errVal = ddxout * activationPartial
        netLyr[l].mn[lk1].ss = errVal 
        #  the below two statements are ?
        ...
        netLyr[l].mn[lk1].applyPtLearnNoActvtn(b_alpha,m_alpha,w_alpha)
    for wk1 in range(len(netLyr[l].wtgrp)):
        netLyr[l].wtgrp[wk1].chkWtContraints()
    ...
    for l in range(1,len(netLyr)-1): # up to (not including) output layer
        ...
        for lk1 in range(len(netLyr[l].mn)):  # current layer
            # first, sum the error terms for multiplet weights, may not be needed here?
            ...
            for lk2 in range(len(netLyr[l+1].mn)):  # from subsequent layer
                errVal = netLyr[l+1].mn[lk2].ddx[lk1] * activationPartial * netLyr[l+1].mn[lk2].ss
                netLyr[l].mn[lk1].ss = errVal
                netLyr[l].mn[lk1].accumTotalE(errVal)   # sum of d(Etotal) /  d Out
...
yTargets.append([1.0,1.0])
yTargets.append([0.0,0.0])
"""

yTargets.append([1.0])
yTargets.append([0.01]) 
yTargets.append([0.99])
yTargets.append([0.98])
...
for l in range(1,2): 
    ...
    i = 0
    netLyr[l].mn.append(multiplet_neuron(netLyr[l].wtgrp[i],-3,1,my_one,0.0)) # "minimum-ish" ( error results with -4 or -5 - don't use)
    netLyr[l].mn.append(multiplet_neuron(netLyr[l].wtgrp[i],1,1,my_one,0.0))  # "dot product"
    netLyr[l].mn.append(multiplet_neuron(netLyr[l].wtgrp[i],5,1,my_one,0.0)) # "max-ish"
    ...
    #netLyr[l].mn.append(multiplet_neuron(netLyr[l].wtgrp[i],-3,1,(-1*my_one),1.0)) # "minimum-ish" ( error results with -4 or -5 - don't use)
    

    #netLyr[l].mn.append(multiplet_neuron(netLyr[l].wtgrp[i],1,1,(-1*my_one),1.0))  # "dot product"
    #netLyr[l].mn.append(multiplet_neuron(netLyr[l].wtgrp[i],5,1,(-1*my_one),1.0)) # "max-ish"
 # weight group for q=2
 #   netLyr[l].wtgrp.append(multiplet_weight_vec(np.ones((vecSize,))*1.0)) 
 #   netLyr[l].mn.append(multiplet_neuron(netLyr[l].wtgrp[1],1,2,my_one,0.0))  #  "multiply-ish"              
 # weight group for q=-1
 #   netLyr[l].wtgrp.append(multiplet_weight_vec(np.ones((vecSize,)))) 
 #   netLyr[l].mn.append(multiplet_neuron(netLyr[l].wtgrp[2],1,-1,my_one,0.0)) # "inverse-ish"
 # weight group for q=-2
 #   netLyr[l].wtgrp.append(multiplet_weight_vec(np.ones((vecSize,)))) 
 #   netLyr[l].mn.append(multiplet_neuron(netLyr[l].wtgrp[2],-1,-2,my_one,0.0)) # "inverse-ish"
    
#    for i in range(len(netLyr[l].wtgrp)):
#        netLyr[l].mn.append(multiplet_neuron(netLyr[l].wtgrp[i],-3,1,my_one,0.0)) # "minimum-ish"
#        netLyr[l].mn.append(multiplet_neuron(netLyr[l].wtgrp[i],-1,-2,my_one,0.0)) # "inverse-ish"
#        netLyr[l].mn.append(multiplet_neuron(netLyr[l].wtgrp[i],1,1,my_one,0.0))  # "dot product"
#        netLyr[l].mn.append(multiplet_neuron(netLyr[l].wtgrp[i],1,2,my_one,0.0)) # "multiplication-ish"
#        netLyr[l].mn.append(multiplet_neuron(netLyr[l].wtgrp[i],5,1,my_one,0.0)) # "max-ish"
...
for l in range(2,3): 
    ...
    # now attach neurons associated with these w groups
    i = 0
    #netLyr[l].mn.append(multiplet_neuron(netLyr[l].wtgrp[i],-3,1,(-1*my_one),1.0))  # b 1 
    netLyr[l].mn.append(multiplet_neuron(netLyr[l].wtgrp[i],5,1,(1*my_one),0.0))  
    netLyr[l].mn.append(multiplet_neuron(netLyr[l].wtgrp[i],1,1,my_one,0.0))  


netLyr.append(mmlp_layer())  # Layer 3
for l in range(3,4): 
    vecSize = len(netLyr[l-1].mn)  # number of neurons in previous layer
    #  define layer weight groups
    # initialize the membership selection weights w_i to 1.0 
    ...
    netLyr[l].mn.append(multiplet_neuron(netLyr[l].wtgrp[i],1,1,my_one,0.0))  
"""
netLyr.append(mmlp_layer())  # Layer 4
for l in range(4,5): 
    vecSize = len(netLyr[l-1].mn)  # number of neurons in previous layer
    #  define layer weight groups
    # initialize the membership selection weights w_i to 1.0 
